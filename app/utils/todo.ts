...
export const todoCreateSchema = z.object({
    title: z.string().min(1, 'Title harus diisi').max(255, 'Title maksimal 255 karakter'),
    description: z.string().max(1000, 'Description maksimal 1000 karakter').optional().or(z.literal('')),
    status: z.enum(['PENDING', 'IN_PROGRESS', 'DONE']).default('PENDING'),
})

export const todoUpdateSchema = z.object({
    title: z.string().min(1, 'Title harus diisi').max(255, 'Title maksimal 255 karakter').optional(),
    description: z.string().max(1000, 'Description maksimal 1000 karakter').optional().or(z.literal('')),
    status: z.enum(['PENDING', 'IN_PROGRESS', 'DONE']).optional(),
})

export const todoBulkCreateSchema = z.object({
    todos: z.array(todoCreateSchema).min(1, 'Minimal 1 todo').max(50, 'Maksimal 50 todo'),
})
